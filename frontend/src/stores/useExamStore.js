...
const useExamStore = create((set, get) => ({
  ...
  fetchSoal: async (courseId, userId) => {
    try {
      ...
      const soalData = res.data.data.map((item) => ({
        ...item,
        opsi: typeof item.opsi === 'string' ? JSON.parse(item.opsi) : Array.isArray(item.opsi) ? item.opsi : [],
      }));
  ...
  handleJawab: async (soalId, answer, userId) => {
    ...
    const jawabanToSend = typeof answer === 'number' ? String.fromCharCode(65 + answer) : answer;
    try {
  ...
  setCurrentIndex: (index) => set((s) => (index >= 0 && index < s.soal.length ? { currentIndex: index } : s)),
}));
